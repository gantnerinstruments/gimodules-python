...
stream_ts_stop = widgets.IntText(description='ts stop:'
,value=measurement_list[len(measurement_list)-1,1])
...
class InputHandler:
    ...
    def in_calc_measurements(self, sel_indices, channels):  # TODO rename function
        ...
        self.sel_channel_1_widgets = [
            widgets.Dropdown(options=temp, description="Param_1:", disabled=False)
            for chan in sel_indices
        ]

        self.sel_calc_widgets = [
            widgets.Dropdown(options=calculations, description="Expression:", diabled=False)
            for chan in sel_indices
        ]

        self.sel_calc_2_widgets = [
            widgets.Dropdown(options=calculations, description="Expression:", diabled=False)
            for chan in sel_indices
        ]

        self.sel_param_widgets = [
            widgets.BoundedFloatText(value=1, description="Constant:", diabled=False)
            for chan in sel_indices
        ]
        ...
        temp2.insert(0, "None")
        self.sel_channel_2_widgets = [
            widgets.Dropdown(options=temp2, description="Param_2:", disabled=False)
            for chan in sel_indices
        ]

        self.sel_unit_widgets = [
            widgets.Text(description="Unit:", disabled=False) for chan in sel_indices
        ]
        ...
        for i in range(len(self.sel_channel_1_widgets)):
            ...
            display(
                self.sel_param_widgets[i],
                self.sel_calc_widgets[i],
                self.sel_channel_1_widgets[i],
                self.sel_calc_2_widgets[i],
                self.sel_channel_2_widgets[i],
                self.sel_unit_widgets[i],
            )
    ...
    def get_expr_param_for_sel_indices(self, sel_indices):
        if not hasattr(self, "expr_param_map"):
            ...
            Expression map example:
             D = {'0': {'exp_c': '/', 'constant': '10', 'exp_2': '*' 'param_2': 'Gi_pyr'}} - 0
             = channel/param_1
            """
        for i in range(len(sel_indices)):  # Not validated : could validate through map first
            self.expr_param_map.update(
                {
                    i: {
                        "param_1": self.sel_channel_1_widgets[i].value,
                        "exp_c": self.sel_calc_widgets[i].value,
                        "exp_2": self.sel_calc_2_widgets[i].value,
                        "constant": self.sel_param_widgets[i].value,
                        "param_2": self.sel_channel_2_widgets[i].value,
                    }
                }
            )
    ...
    def get_channel_index_for_data(self, map_index, param, channel):
        if channel == 1:
            return self.sel_channel_1_widgets[map_index].options.index(
                self.expr_param_map[map_index][param]
            )
        elif channel == 2:
            return self.sel_channel_2_widgets[map_index].options.index(
                self.expr_param_map[map_index][param]
            )
    ...
    def display_stream_channel_sel(self, conn_cloud, default_stream=0):
        ...
        # map sources
        conn_cloud.variable_mapping(stream_ID_wid.value)
        index_var, name_var, unit_var, id_var = conn_cloud.get_var_mapping(
            conn_cloud.request_map_res
        )
        ...
        print(
            "Choose the Stream and Variables of which you want to fetch data."
    ...
    def get_selected_channel_indices(self, index_var, name_var):
        selected_index = [
            index_var[name_var.index(self.wid_channel.value[i])]
            for i in range(0, len(self.wid_channel.value))
        ]
    ...
    def display_date_pickers(self):
        ...
        start_date_pick = widgets.DatePicker(
            value=a_week_ago, description="Start Date", disabled=False
        )

        end_date_pick = widgets.DatePicker(value=today, description="End Date", disabled=False)
